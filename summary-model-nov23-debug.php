...
$role_added_depts2 = array();
// if($user_id == 1252020 ) $node = px_h3_v3::get_department(128,1); //AND user_id != 5140 // need modify 212,213  72,73,200,202
$sql = "SELECT h3_ids FROM role_details WHERE user_id = $user_id AND role_lkp_id IN (212,213,72,73,200,202) AND x = 'active' AND user_id != 5140  "; 
$db->query($sql);
...
// need modify bali ubrahan sang duplicate nga role dept kag team

// $sql2 = "SELECT h3_ids FROM role_details WHERE user_id = $user_id AND role_lkp_id IN (212,213) AND x = 'active'  "; 
// $db->query($sql2);
// $result2 = $db->fetch_assoc();
// if (!empty($result2) && $result['h3_ids'] != 0) {
// 	$hid2 = array_filter(explode(",", $result['h3_ids']));
// 	$node2 = px_h3_v3::get_department($hid[0], 1);

// 	foreach($hid2 as $ht_id2) $role_added_depts2[$ht_id2]=1;

// }

$dept_sql = "SELECT h3_ids FROM role_details WHERE user_id = $user_id AND role_lkp_id IN (212) AND x = 'active'"; 
$db->query($dept_sql);
$row_dept = $db->fetch_assoc();


$team_sql = "SELECT h3_ids FROM role_details WHERE user_id = $user_id AND role_lkp_id IN (213) AND x = 'active'"; 
$db->query($team_sql);
$row_team = $db->fetch_assoc();
...
// need modify to not fixed modify
if($role_added_depts && in_array($user_id,array( 52483,1251579,1240374,1253226,1253234,4962 ))){ //carmi, julie morales, 1240374 kristia, 1253226 bradfordd, 1253234 Sara MT CSM of IAO, 4962 Melisa timbol
    foreach($role_added_depts as $ht_id => $null){
        ...
        // need modify to not fixed bali removed ni 129 to 142 - louie 
        // if($ht_id == 505){ 
        // // colombia itps, then add colombia trainees
...
// need modify
	//if (in_array($user_id, array(1253415, 1252947, 1252979))) // fernanda, juan manuel
		// $hquery_id [] = 520;

	if(!empty($row_team['h3_ids'])){
		$hquery_id[] = $row_team['h3_ids'];
	}
...
if (join(",", $downline_h3_ids) != '') {
	...
	IN (" . join(",", $downline_h3_ids) . "{$include} ,".$row_dept['h3_ids'].") AND node_type ='group' AND `hierarchy_tree_id`  NOT IN (13,14) 
...
// eeh merge ni ang in array sila duwa modify
...
if (count($groups) >= 1) {
	foreach ($groups as $gid => $grp) :
		...
		$str = implode(',', $downline_h3_ids).",".$row_dept['h3_ids'];
		$groups[$str] = 'All '.$row_team['h3_ids'];
